...
def build_initial_question_prompt(topic: str, chapter_number: int | None, question_count: int) -> str:
    chapter_line = f"Chapter number: {chapter_number}" if chapter_number is not None else "Chapter number: not specified"
    return (
        "You are an oral viva examiner. Generate the first viva question and expected key points as JSON.\n"
...
def build_question_bank_prompt(topic: str, chapter_number: int | None, question_count: int) -> str:
    chapter_line = f"Chapter number: {chapter_number}" if chapter_number is not None else "Chapter number: not specified"
    return (
        "You are an oral viva examiner. Generate a full base question bank for this session.\n"
...
def build_followup_prompt(topic: str, previous_question: str, answer_transcript: str, question_index: int, total_questions: int) -> str:
    return (
        "You are an oral viva examiner. Generate the next question as JSON.\n"
...
def build_answer_evaluation_prompt(question: str, expected_points: list[str], answer_transcript: str) -> str:
    return (
        "Evaluate this oral viva answer using the expected points.\n"
